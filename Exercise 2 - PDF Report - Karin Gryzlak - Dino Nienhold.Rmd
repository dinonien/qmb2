...
We are 95% confident that the serice times are between `r me + c(-1) * qnorm(.975) * se` and `r me + c(1) * qnorm(.975) * se`.

###d)

A t-test is conducted to see if the mean of x is less than four hours.
...
Figure 1 shows that the distribution is right skewed with several outliers on the higher rps range. 
Skewness value is `r skewness(nrehousing$rps)`.
Figure 2 shows a qq plot. The plot does also show the outliers and the non-normality of the distribution. 
...
Figure 3 shows that the distribution is right skewed with several outliers on the higher rps range. 
Skewness value is `r skewness(nonnrehousing$rps)`.
Figure 4 shows a qq plot. The plot does also show the outliers and the non-normality of the distribution.
...
In the second t-test equal variance is  assumed. The p-value is `r rpsTTestGreater$p.value` and much higher than 0.05. 
...
http://geneseo.edu/~bearden/socl211/chisquareweb/chisquare.html

```{r}

housingrentTbl <- xtabs(~rooms+nre, data=housingrents)
housingrentChi <- chisq.test(housingrentTbl,simulate.p.value=TRUE) 
housingrentChi
housingrentChi$observed
housingrentChi$expected
housingrentChi$resid
resid(housingrentChi)
